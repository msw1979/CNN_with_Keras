...
def CNN_model(input_shape, num_classes):
    ...
    #flattening
    model.add(Flatten())
    #add dense layer with 100 neurons and relu activation
    model.add(Dense(100, activation='relu'))
    #add dense layer with num_classes neurons and softmax activation
...
def data_CNN(x_train,y_train,x_test,y_test):
...
#print
print('x train shape: {} and y train shape: {}'.format(X_train.shape, X_train.shape), file=output_file)
print('x test shape: {} and y test shape: {}'.format(X_test.shape, X_test.shape), file=output_file)
...
#CNN training
print("CNN Model", file=output_file)
x_train, y_train, x_test, y_test, num_classes = data_CNN(X_train,Y_train,X_test,Y_test)
...
# fit the model
results = CNN.fit(x_train, y_train, validation_data=(x_test, y_test), epochs=20, batch_size=200, verbose=2)
...
plot_outpput(predictions, X_test, Y_test, 'CNN')
plot_loss_accuracy(train_loss, val_loss, train_acc, val_acc, 'CNN')

#save and load
#model.save('CNN_model.h5')
#pretrained_model = load_model('convolution_model.h5')
